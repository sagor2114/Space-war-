...
public class SpacePanel extends JPanel implements Runnable, KeyListener {
	...
	public void run () {
		...
		String score = "Total score: " + (player1.getScore() + newPlayer.getScore());
		...
		try {
			if (player1.getScore() > Integer.parseInt(arr[1])) {
				//		Write the 'score' file with the new scores
				writeFile(scoreFile, waveNumber + " " + (player1.getScore() + newPlayer.getScore()));

				maxScore = "Best record \nLevel: " + waveNumber + " | Score: " + (player1.getScore() + newPlayer.getScore());
		...
		g.drawString("", (width - length) / 2 + 270, (height - length) / 2 + 340);
		gameDraw();
	}
	...
	private void gameUpdate() {
		...
		for (int i = 0; i < enemies.size(); i++) {
			if (enemies.get(i).isDead()) {
				...
				player1.setScore(e.getType() + e.getRank());
				enemies.remove(i);
				i--;
		...
		if (!newPlayer.isRecovering()) {
			int px = newPlayer.getX();
			int py = newPlayer.getY();
			int pr = newPlayer.getR();
			for (int i = 0; i < enemies.size(); i++) {
				Enemy e = enemies.get(i);
				double ex = e.getX();
				double ey = e.getY();
...
//					player.loseLife();
		...
		for (int i = 0; i < powerUps.size(); i++) {
			...
			if (dist < pr + r) {
				int type = p.getType();
				if (type == 1) {
					player1.setLife(player1.getLives() + 1);
					texts.add(new Text(player1.getX(), player1.getY(), 2000, "Life +1"));
				}
				if (type == 2) {
					player1.increasePower(1);
					texts.add(new Text(player1.getX(), player1.getY(), 2000, "Power +1"));
				}
				if (type == 3) {
					player1.increasePower(2);
					texts.add(new Text(player1.getX(), player1.getY(), 2000, "Power +2, you drink a Monster Energy"));
				}
	...
	Timer timer;
	static boolean isSpecialPlaying = false;
	void setTimer() {
		timer = new Timer();
	...
	private void gameRender() {
		...
		g.fillRect(20, 60, player1.getPower() * 8, 8);
		g.setColor(Color.YELLOW.darker());
		g.setStroke(new BasicStroke(3));
		for (int i = 0; i < player1.getRequiredPower(); i++)
		...
		g.drawString("Score: " + player1.getScore(), width - 150, 30);
	...
	public void keyPressed (KeyEvent e) {
		...
		if (keyCode == KeyEvent.VK_LEFT) player1.setLeft(true);
		if (keyCode == KeyEvent.VK_RIGHT) player1.setRigth(true);
		if (keyCode == KeyEvent.VK_UP) player1.setUp(true);
		if (keyCode == KeyEvent.VK_DOWN) player1.setDown(true);
		if (keyCode == KeyEvent.VK_Z || keyCode == 32) player1.setFiring(true);
		
		if (keyCode == 27) System.exit(0);
		if (keyCode == KeyEvent.VK_A && isSpecialPlaying) newPlayer.setLeft(true);
		if (keyCode == KeyEvent.VK_D && isSpecialPlaying) newPlayer.setRigth(true);
		if (keyCode == KeyEvent.VK_W && isSpecialPlaying) newPlayer.setUp(true);
		if (keyCode == KeyEvent.VK_S && isSpecialPlaying) newPlayer.setDown(true);
		if ((keyCode == KeyEvent.VK_Z || keyCode == 32) && isSpecialPlaying) newPlayer.setFiring(true);
...
//		}
	...
	public void keyReleased (KeyEvent e) {
		...
		if (keyCode == 27) System.exit(0);
		if (keyCode == KeyEvent.VK_LEFT) player1.setLeft(false);
		if (keyCode == KeyEvent.VK_RIGHT) player1.setRigth(false);
		if (keyCode == KeyEvent.VK_UP) player1.setUp(false);
		if (keyCode == KeyEvent.VK_DOWN) player1.setDown(false);
		if (keyCode == KeyEvent.VK_Z || keyCode == 32) player1.setFiring(false);
		
		if (keyCode == 27) System.exit(0);
		if (keyCode == KeyEvent.VK_A) newPlayer.setLeft(false);
		if (keyCode == KeyEvent.VK_D) newPlayer.setRigth(false);
		if (keyCode == KeyEvent.VK_W) newPlayer.setUp(false);
		if (keyCode == KeyEvent.VK_S) newPlayer.setDown(false);
		if (keyCode == KeyEvent.VK_Z || keyCode == 32) newPlayer.setFiring(false);
	}
